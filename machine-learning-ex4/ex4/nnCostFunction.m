...
ylog = -labels.*log(a3);
part2 = -(1-labels).*log(1-a3);
bracket = ylog + part2;
J = sum(sum(bracket))/m;

%Regularization to the Cost Function 
 theta1 = sum(sum(Theta1(:, 2:end).^2));
 theta2 = sum(sum(Theta2(:, 2:end).^2));
 reg = lambda*(theta1+theta2)/(2*m);
 J = J+reg;
